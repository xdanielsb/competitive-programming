...
inline point r(point o, point d){ return {d.x-o.x, d.y - o.y} ;} //Diference between two vectors
inline ld dist(point o, point d){ return  sqrt(dot(r(o,d) , r(o,d))); }

inline  bool segments_parallel(point a, point b, point c){ // c is a point of ab
...
inline bool point_on_segment(verts v,  point c){
    ...
    for (int i=0;i<cant;i++){
        ...
        if(segments_parallel(v[i], v[(i+1)%cant], c) && dot(r(c,v[i]), r(c,v[(i+1)%cant])) < 0) {
                return true;
        }
    }
    return false;
...
bool point_in_polygon(point p, verts a){
    ...
    for (int i=0;i<cant;i++){
        int j = (i+1) % cant;
        if ( (p.y < a[i].y != p.y < a[j].y) && (p.x < a[i].x + (a[j].x-a[i].x)*(p.y-a[i].y)/(a[j].y-a[i].y)) ){
            inside = !inside;
        }
    }
    return inside;
